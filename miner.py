#!/usr/bin/env python3
"""
RAMCOIN MINER v7.0.0 — MAXIMUM + ANTI-SERVER
Авто-настройка под любой процессор.
Защита от ASIC, GPU, серверных CPU.
Минималистичный вывод — без мерцания.
"""
...
NODES = [
    {"http": "http://localhost:5000", "ws": "ws://localhost:5000/ws"},
]
...
class HardwareDetector:
        ...
        try:
            if platform.system() == "Linux":
                ...
                if phys_ids:
                    ...
                    if cores_per_socket:
                        return len(phys_ids) * int(cores_per_socket.pop())
            if HAS_PSUTIL:
                                        ...
                                        if f.read().strip() == '3':
                                            if os.path.exists(size_path):
                                                with open(size_path) as f:
                                                    l3 = max(l3, self._parse_size(f.read().strip()))
                if l3 > 0:
                    return l3
        except:
            pass
    ...
    def _check_server(self):
        ...
        for kw in server_kw:
            if kw in cpu_lower:
                detected = True
                self.server_reasons.append(f"Server CPU: {kw.upper()}")
                break
        if 'threadripper' in cpu_lower and 'pro' in cpu_lower:
            detected = True
            self.server_reasons.append("Threadripper PRO (server)")
        if 'threadripper' in cpu_lower and 'pro' not in cpu_lower:
            detected = True
            self.server_reasons.append("Threadripper (HEDT)")
        if self.physical_cores > MAX_HOME_CORES:
            detected = True
            self.server_reasons.append(f"Cores: {self.physical_cores} > {MAX_HOME_CORES}")
        if self.l3_cache_mb > MAX_HOME_L3_MB:
            detected = True
            self.server_reasons.append(f"L3: {self.l3_cache_mb}MB > {MAX_HOME_L3_MB}MB")

        home_whitelist = ['ryzen 9 7950x', 'ryzen 9 5950x', 'ryzen 9 3950x',
                          'i9-14900k', 'i9-13900k', 'i9-12900k',
                          'ryzen 7 7800x3d', 'ryzen 7 5800x3d', 'core ultra 9']
        if detected:
            for hw in home_whitelist:
                if hw in cpu_lower and self.physical_cores <= 16:
                    detected = False
                    self.server_reasons = ["High-end home PC (whitelisted)"]
                    break
        self.is_server = detected

    def _configure(self):
        if self.is_server:
            self.recommended_threads = MAX_SERVER_THREADS
            self.server_penalty = SERVER_PENALTY
            self.scratchpad_size = BASE_BUFFER_SIZE * 2
        else:
...
class MinerState:
    ...
    def update_speed(self, tid, sols):
        with self.lock:
            ...
            if self.speed == 0:
                ...
                self.speed = self.speed * SPEED_SMOOTHING + instant * (1 - SPEED_SMOOTHING)
            self.thread_speeds[tid] = instant
            self.thread_updates[tid] = now

    def thread_heartbeat(self, tid):
        with self.lock:
            self.thread_heartbeats[tid] = time.time()
    ...
    def get_template(self):
        with self.template_lock:
            return (self.height, self.prev_hash, self.target, self.reward,
                    self.txs, self.template_version, self.current_buffer_size)

    def get_node_url(self):
        return NODES[self.primary_node]["http"]

    def get_ws_url(self):
        return NODES[self.primary_node]["ws"]

    def switch_node(self):
        with self.lock:
            self.primary_node = (self.primary_node + 1) % len(NODES)
        print(f"  🔄 Node: {self.get_node_url()}")
        ...
        try:
            ...
            if e.code == 400:
                try:
                    ...
                    last_err = body.get("reason", last_err)
                except:
                    pass
        except urllib.error.URLError as e:
            last_err = f"URL: {str(e.reason)[:40]}"
...
def sign_block(block, priv_hex):
    try:
        ...
        data = {
            ...
            "nonce_seed": block.get("nonce_seed", 0), "memory_proof": block["memory_proof"],
            "miner_payout_address": block["miner_payout_address"],
            "scratchpad_mods": block.get("scratchpad_mods", 0)
        }
        sig = priv.sign(json.dumps(data, sort_keys=True).encode(), ec.ECDSA(hashes.SHA256()))
        pub = priv.public_key()
        pub.verify(sig, json.dumps(data, sort_keys=True).encode(), ec.ECDSA(hashes.SHA256()))
        return sig.hex()
    except Exception as e:
        state.add_error(f"Sign: {e}")
        return None
...
def create_scratchpad(prev_hash, tid, nseed, buffer_size=None):
    ...
    material = f"{prev_hash}|{tid}|{nseed}|RAMCOIN_v7|{size}"
...
def memhard_loop(sp, seed, nonce, nseed, buffer_size=None):
    ...
    for k in range(SCRATCHPAD_ITER):
        ...
        if k > 0 and k % 50000 == 0:
            nseed = (nseed + 1) & 0xFFFFFFFF
            mix = (mix ^ nseed) & 0xFFFFFFFFFFFFFFFF
...
class MinerThread:
    ...
    def run(self):
        if HAS_PSUTIL:
            try:
                ...
                target_cpu = self.tid % phys if phys else self.tid % cpu_count
        ...
        while state.active:
            try:
                ...
                state.add_error(f"T{self.tid}: {e}")
                time.sleep(1)
                self.nonce = secrets.randbits(32)
                self.nseed = secrets.randbits(16)
    ...
    def _mine_loop(self):
        ...
        while state.active and not state.block_found:
            if hw.is_server and self.skip_rate > 1:
                self.skip_counter += 1
                if self.skip_counter % self.skip_rate != 0:
            ...
            state.thread_heartbeat(self.tid)
            ...
            try:
                ...
                if is_valid:
                    ...
                    block = {
                        ...
                        "transactions": txs, "timestamp": int(time.time()),
                        "nonce": int(self.nonce), "nonce_seed": int(self.nseed),
                        "memory_proof": proof, "target": target,
                        "extra_nonce": self.tid, "miner_payout_address": self.address,
                        "scratchpad_mods": mods, "scratchpad_size": actual_size
                    }

                    sig = sign_block(block, self.priv_key)
                    if not sig:
                        with state.lock: state.block_found = False
                        self.nonce = secrets.randbits(32)
                        continue
                    ...
                    with state.lock:
                        if block_hash in state.submitted_blocks:
                    ...
                    submitted = self._submit_block(block, height, reward)
                    if not submitted:
                        state.switch_node()
                        submitted = self._submit_block(block, height, reward)
                    ...
                    if submitted:
                        ...
                        print(f"\n  {GREEN}✅ BLOCK #{height} ACCEPTED! +{reward:.2f} RAM{RESET}")
                    else:
                        with state.lock:
                            state.rejected += 1
                            state.block_found = False
                        print(f"\n  {RED}❌ BLOCK #{height} REJECTED{RESET}")
                    return
    ...
    def _submit_block(self, block, height, reward):
        for attempt in range(3):
            resp = safe_request(f"{state.get_node_url()}/mine", block, timeout=SUBMIT_TIMEOUT, max_retries=1)
            if resp and resp.get("status") == "ok":
                return True
            if resp:
                reason = resp.get("reason", "unknown")
                if reason in ["wrong_index", "invalid_previous_hash"]:
                    return False
...
def logger(threads, address):
    ...
    while True:
        ...
        if now - last_log >= LOG_INTERVAL:
            ...
            status = "🟢" if (s.active and s.node_ok) else "🟡" if s.node_ok else "🔴"
            print(f"[{fmt_time(uptime)}] {status} {fmt_speed(s.speed)} | "
                  f"⛏️ {s.mined} blocks | ❌ {s.rejected} | "
                  f"H:#{s.height} | 💰 {s.reward:.2f} RAM | "
                  f"🧵 {threads}t", flush=True)
            last_log = now
...
def ws_client():
    while True:
        try:
            url = state.get_ws_url()
            ws = websocket.create_connection(url, timeout=10, ping_interval=HEARTBEAT_INTERVAL, ping_timeout=5)
            state.ws_ok = True
            state.node_ok = True
            ws.send(json.dumps({
                "type": "miner", "version": VERSION,
                "hardware": {
                    "cpu": hw.cpu_model, "vendor": hw.cpu_vendor,
    ...
    for wf in ["ramcoin_wallet.json", "wallet.json", os.path.expanduser("~/.ramcoin/wallet.json")]:
        ...
        try:
            ...
            if "private_key_hex" in data:
                if len(data["private_key_hex"]) == 64 and data.get("address","").startswith("RAM_"):
...
# ═══════════════════ MAIN ═══════════════════
def main():
    print(f"\n  {CYAN}{BOLD}RAMCOIN MINER v{VERSION}{RESET}")
    print(f"  {DIM}CPU: {hw.cpu_model[:55]}{RESET}")
    print(f"  {DIM}{hw.cpu_vendor.upper()} | {hw.physical_cores}P/{hw.total_cores}L | L3: {hw.l3_cache_mb}MB{RESET}")

    if hw.is_server:
        print(f"  {RED}⚠️ SERVER — Penalty: {int((1-hw.server_penalty)*100)}%{RESET}")
    else:
        print(f"  {GREEN}✅ HOME PC — Full hashrate{RESET}")

    threads = hw.get_effective_threads()
    mem_mb = hw.get_thread_memory_mb()
    print(f"  🧵 Threads: {threads} | 💾 {mem_mb:.0f}MB/thread = {mem_mb*threads:.0f}MB total")
    print(f"  📡 Node: {NODES[0]['http']}")
    print()
    ...
    def sig_handler(sig, frame):
        print(f"\n  ⛏️ {state.mined} blocks | 💰 {state.total_ram:.4f} RAM | ❌ {state.rejected}")
        state.active = False; state.block_found = True
    ...
    print(f"  ⛏️  Mining started... (Ctrl+C to stop)\n")
