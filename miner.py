#!/usr/bin/env python3
"""
RAMCOIN MINER v7.0.2 — SOLO + POOL
Основа: рабочий v7.0. Выбор режима при запуске.
"""
...
class HardwareDetector:
    def __init__(self):
        self.is_server = False; self.server_reasons = []
        self.total_cores = os.cpu_count() or 2
        self.physical_cores = self._get_physical_cores()
        self.l3_cache_mb = 0; self.cpu_model = ""; self.cpu_vendor = ""
        self.recommended_threads = 2; self.scratchpad_size = BASE_BUFFER_SIZE; self.server_penalty = 1.0
        self._detect()

    def _get_physical_cores(self):
        try:
            if platform.system() == "Linux":
                with open('/proc/cpuinfo') as f: content = f.read()
    ...
    def _detect(self):
        ...
        except: self.cpu_model = platform.processor() or "Unknown"
        cpu_lower = self.cpu_model.lower()
        if any(k in cpu_lower for k in ['amd', 'ryzen', 'athlon', 'epyc']): self.cpu_vendor = "amd"
        elif any(k in cpu_lower for k in ['intel', 'xeon', 'core', 'pentium', 'celeron']): self.cpu_vendor = "intel"
        elif any(k in cpu_lower for k in ['apple', 'm1', 'm2', 'm3', 'm4']): self.cpu_vendor = "apple"
        else: self.cpu_vendor = "unknown"
        self.l3_cache_mb = self._get_l3_cache(); self._check_server(); self._configure()
                    ...
                    if cpu_dir.startswith('cpu') and cpu_dir[3:].isdigit():
                        ...
                        if os.path.exists(cache_path):
                            for idx in os.listdir(cache_path):
                                level_path = f'{cache_path}/{idx}/level'; size_path = f'{cache_path}/{idx}/size'
                                if os.path.exists(level_path):
                                    with open(level_path) as f:
                                        if f.read().strip() == '3':
                                            if os.path.exists(size_path):
                                                with open(size_path) as f: l3 = max(l3, self._parse_size(f.read().strip()))
                if l3 > 0: return l3
        except: pass
        cpu_l = self.cpu_model.lower()
        if 'ryzen 9 79' in cpu_l or 'ryzen 9 59' in cpu_l: return 64
        elif 'ryzen 9' in cpu_l: return 32
        elif 'ryzen 7 78' in cpu_l or 'ryzen 7 58' in cpu_l: return 32
        elif 'ryzen 7' in cpu_l: return 16
        elif 'ryzen 5 56' in cpu_l or 'ryzen 5 76' in cpu_l: return 32
        elif 'ryzen 5' in cpu_l: return 16
        elif 'ryzen 3' in cpu_l: return 8
        elif 'i9-' in cpu_l: return 30
        elif 'i7-' in cpu_l: return 24
        elif 'i5-' in cpu_l: return 18
        elif 'i3-' in cpu_l: return 12
        return 8

    def _parse_size(self, size_str):
        size_str = size_str.strip().upper()
        if size_str.endswith('K'): return int(size_str[:-1]) // 1024
        elif size_str.endswith('M'): return int(size_str[:-1])
        elif size_str.endswith('G'): return int(size_str[:-1]) * 1024
        else:
            try: return int(size_str) // (1024 * 1024)
            except: return 0
    ...
    def _check_server(self):
        ...
        home_whitelist = ['ryzen 9 7950x', 'ryzen 9 5950x', 'ryzen 9 3950x', 'i9-14900k', 'i9-13900k', 'i9-12900k', 'ryzen 7 7800x3d', 'ryzen 7 5800x3d', 'core ultra 9']
        if detected:
    ...
    def _configure(self):
        if self.is_server: self.recommended_threads = MAX_SERVER_THREADS; self.server_penalty = SERVER_PENALTY; self.scratchpad_size = BASE_BUFFER_SIZE * 2
        else:
            optimal = max(2, min(MAX_HOME_THREADS, self.l3_cache_mb // 4))
            optimal = min(optimal, self.physical_cores); optimal = min(optimal, self.total_cores)
            self.recommended_threads = optimal; self.server_penalty = 1.0; self.scratchpad_size = BASE_BUFFER_SIZE

    def get_effective_threads(self): return self.recommended_threads
    def get_thread_memory_mb(self): return self.scratchpad_size * 8 / (1024 * 1024)
...
class MinerState:
    def __init__(self):
        ...
        self.height = -1; self.prev_hash = ""; self.target = 0; self.reward = 10.0
        self.txs = []; self.active = False; self.block_found = False
        self.node_ok = False; self.ws_ok = False; self.primary_node = 0
        self.mined = 0; self.total_ram = 0.0; self.rejected = 0
        self.start_time = time.time(); self.speed = 0.0
        self.thread_speeds = {}; self.thread_updates = {}
        self.template_version = 0; self.template_lock = threading.RLock()
        self.submitted_blocks = set()
        self.current_buffer_size = hw.scratchpad_size
        self.pool_mode = False; self.pool_shares = 0

    def update_speed(self, tid, sols):
        with self.lock:
            now = time.time(); last = self.thread_updates.get(tid, now - 1)
            elapsed = max(0.01, now - last); instant = sols / elapsed
            self.speed = instant if self.speed == 0 else self.speed * SPEED_SMOOTHING + instant * (1 - SPEED_SMOOTHING)
            self.thread_speeds[tid] = instant; self.thread_updates[tid] = now

    def get_template(self):
        with self.template_lock:
            return (self.height, self.prev_hash, self.target, self.reward, self.txs, self.template_version, self.current_buffer_size)
...
def safe_request(url, data=None, timeout=NODE_TIMEOUT, max_retries=MAX_RETRIES):
    ...
    for attempt in range(max_retries):
        try:
            if data is None: req = urllib.request.Request(url, headers={"Connection": "close"})
            else: req = urllib.request.Request(url, data=json.dumps(data).encode(), headers={"Content-Type": "application/json", "Connection": "close"}, method="POST")
            with urllib.request.urlopen(req, timeout=timeout) as r: state.node_ok = True; return json.loads(r.read().decode())
        except: time.sleep(RETRY_BASE_DELAY * (2 ** attempt))
    state.node_ok = False; return None
...
def sign_block(block, priv_hex):
    try:
        ...
        data = {"index": block["index"], "previous_hash": block["previous_hash"], "timestamp": block["timestamp"],
                ...
                "miner_payout_address": block["miner_payout_address"], "scratchpad_mods": block.get("scratchpad_mods", 0)}
...
class MinerThread:
    """Соло-майнер (проверенная логика)"""
    def __init__(self, tid, address, priv_key):
        self.tid = tid; self.address = address; self.priv_key = priv_key
        self.nonce = secrets.randbits(32); self.nseed = secrets.randbits(16)
        self.iterations = 0; self.last_restart = time.time(); self.thread = None
        self.skip_counter = 0
    ...
    def run(self):
        if HAS_PSUTIL:
            try:
                p = psutil.Process(); cpu_count = psutil.cpu_count(logical=True)
    ...
    def _mine_loop(self):
        ...
        try: sp, seed, actual_size = create_scratchpad(prev_hash, self.tid, self.nseed, buf_size)
        except: time.sleep(0.5); return

        local_sol = 0; last_report = time.time()
        while state.active and not state.block_found:
            ...
            if cur[5] != tpl_ver:
                ...
                self.nonce = secrets.randbits(32); self.nseed = secrets.randbits(16); self.iterations = 0

            if time.time() - self.last_restart > BLOCK_TIMEOUT:
                self.nonce = secrets.randbits(32); self.nseed = secrets.randbits(16); self.last_restart = time.time()
                try: sp, seed, actual_size = create_scratchpad(prev_hash, self.tid, self.nseed, buf_size)
                except: return
            ...
            try:
                ...
                local_sol += 1; self.iterations += 1
                now = time.time()
                if now - last_report >= SPEED_UPDATE_INTERVAL: state.update_speed(self.tid, local_sol); local_sol = 0; last_report = now
                ...
                if is_valid:
                    ...
                    block = {"index": height, "previous_hash": prev_hash, "transactions": txs, "timestamp": int(time.time()),
                             "nonce": int(self.nonce), "nonce_seed": int(self.nseed), "memory_proof": proof, "target": target,
                             "extra_nonce": self.tid, "miner_payout_address": self.address, "scratchpad_mods": mods, "scratchpad_size": actual_size}
                ...
                if self.nonce > 2**48: self.nonce = secrets.randbits(32); self.nseed = secrets.randbits(16); self.iterations = 0
            except: self.nonce = secrets.randbits(32); time.sleep(0.1)


class PoolThread:
    """Пул-майнер"""
    def __init__(self, tid, address):
        self.tid = tid; self.address = address
        self.nonce = secrets.randbits(32); self.nseed = secrets.randbits(16)
        self.last_restart = time.time(); self.thread = None
    ...
    def _pool_loop(self):
        while state.active and not state.block_found:
            ...
            prev_hash = tmpl["previous_hash"]; pool_target = tmpl.get("pool_target", state.target * 100)
            ...
            while state.active and not state.block_found:
                ...
                try:
                    ...
                    if is_share:
                        share_data = {"miner_address": self.address, "nonce": int(self.nonce), "nonce_seed": int(self.nseed),
...
def logger(threads, address):
    ...
    while True:
        ...
        if now - last_log >= LOG_INTERVAL:
            s = state; mode = "POOL" if s.pool_mode else "SOLO"
            print(f"[{fmt_time(time.time() - s.start_time)}] {mode} | {fmt_speed(s.speed)} | Blocks: {s.mined} | Rejected: {s.rejected} | Height: #{s.height} | Reward: {s.reward:.2f} RAM", flush=True)
            last_log = now
...
def ws_client():
    while True:
        try:
            ws = websocket.create_connection(state.get_ws_url(), timeout=10, ping_interval=HEARTBEAT_INTERVAL, ping_timeout=5)
            state.ws_ok = True; state.node_ok = True
            ws.send(json.dumps({"type": "miner", "version": VERSION, "hardware": {"cpu": hw.cpu_model, "vendor": hw.cpu_vendor, "cores_physical": hw.physical_cores, "cores_logical": hw.total_cores, "l3_mb": hw.l3_cache_mb, "is_server": hw.is_server}}))
            while True:
                try:
                    ...
                    if data.get("event") == "new_block":
                        ...
                        if chain:
                            with state.template_lock: state.height = chain["length"]; state.prev_hash = chain["chain"][-1]["hash"]; state.target = chain.get("current_target", 0); state.reward = 10.0 / (2 ** (state.height // 876000)); state.txs = safe_request(f"{state.get_node_url()}/pending") or []; state.template_version += 1
                except websocket.WebSocketTimeoutException: continue
                except: break
        except: state.ws_ok = False; time.sleep(WS_RECONNECT_DELAY)
...
def load_wallet():
    for wf in ["ramcoin_wallet.json", "wallet.json", os.path.expanduser("~/.ramcoin/wallet.json")]:
        ...
        try:
            with open(wf) as f: data = json.load(f)
            if "private_key_hex" in data and len(data["private_key_hex"]) == 64 and data.get("address","").startswith("RAM_"): return data["private_key_hex"], data["address"]
            if "crypto_data" in data:
                for i in range(3):
                    pw = input(f"Password ({i+1}/3): ").strip()
                    if not pw: continue
                    try:
                        raw = bytes.fromhex(data["crypto_data"]); key = pbkdf2_hmac('sha512', pw.encode(), raw[:32], 600000, dklen=32)
                        cipher = AES.new(key, AES.MODE_GCM, nonce=raw[32:48])
                        dec = json.loads(cipher.decrypt_and_verify(raw[64:], raw[48:64]))
                        if len(dec["private_key_hex"])==64 and dec["address"].startswith("RAM_"): return dec["private_key_hex"], dec["address"]
                    except: pass
        except: pass
    return None, None
...
def main():
    ...
    if hw.is_server: print(f"SERVER — Penalty: {int((1-hw.server_penalty)*100)}%")
    else: print(f"HOME PC — Full hashrate")
    ...
    try:
        libc = ctypes.CDLL(ctypes.util.find_library("c"), use_errno=True); libc.mlockall(1 | 2)
    except: pass

    def sig_handler(sig, frame):
        print(f"\nBlocks: {state.mined} | RAM: {state.total_ram:.4f} | Rejected: {state.rejected} | Shares: {state.pool_shares}")
        state.active = False; state.block_found = True; time.sleep(0.5); sys.exit(0)
    ...
    with state.template_lock:
        state.height = chain["length"]; state.prev_hash = chain["chain"][-1]["hash"]
        state.target = chain.get("current_target", 0); state.reward = 10.0 / (2 ** (state.height // 876000))
        state.txs = safe_request(f"{state.get_node_url()}/pending") or []; state.template_version = 1

    state.active = True
    print(f"Height: #{state.height} | Diff: {fmt_speed(MAX_TARGET/max(1, state.target))} | Reward: {state.reward} RAM\n")

    miner_threads = []
    if state.pool_mode:
        for tid in range(threads):
            pt = PoolThread(tid, address); t = threading.Thread(target=pt.run, daemon=True, name=f"p-{tid}")
            t.start(); pt.thread = t; miner_threads.append(pt)
    else:
        for tid in range(threads):
            mt = MinerThread(tid, address, priv_hex); t = threading.Thread(target=mt.run, daemon=True, name=f"m-{tid}")
            t.start(); mt.thread = t; miner_threads.append(mt)

    while True:
        time.sleep(1)
        if state.block_found:
            state.block_found = False; state.active = False; time.sleep(0.3)
...
if __name__ == "__main__":
    ...
    except Exception as e: print(f"Fatal: {e}"); traceback.print_exc(); sys.exit(1)