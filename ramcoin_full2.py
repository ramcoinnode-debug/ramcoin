...
"""
╔══════════════════════════════════════════════════════╗
║   RAMCOIN NODE v10.3.0 — MAXIMUM SECURITY EDITION   ║
║   Защита: ChaCha20-Poly1305 + ECDH + ECDSA          ║
║   Сеть: P2P + SSL/TLS + Seed-ноды + RateLimit       ║
...
class SeedRegistry:
    ...
    def _init_db(self):
        with sqlite3.connect(SEEDS_DB) as conn:
            conn.execute('''CREATE TABLE IF NOT EXISTS seeds (addr TEXT PRIMARY KEY, ip TEXT, port INTEGER, reputation REAL, last_seen REAL, height INTEGER, version TEXT)''')
            conn.execute('PRAGMA journal_mode=WAL')

    async def add_seed(self, ip: str, port: int, height: int, version: str = "") -> bool:
        async with self.lock:
            if not IPUtils.is_public_ip(ip):
                return False
            addr = f"{ip}:{port}"
            now = time.time()
            self.seeds[addr] = PeerInfo(addr=addr, ip=ip, port=port, last_seen=now, height=height, version=version)
    ...
    async def _save_seed(self, addr: str):
        try:
            ...
            with sqlite3.connect(SEEDS_DB, timeout=10) as conn:
                conn.execute("INSERT OR REPLACE INTO seeds VALUES (?,?,?,?,?,?,?)", (addr, info.ip, info.port, self.reputation.get(addr, 0), info.last_seen, info.height, info.version))
        except:
            pass
...
class Blockchain:
    ...
    def _load(self):
        try:
            if not os.path.exists(DB_PATH):
                return False
            with sqlite3.connect(DB_PATH) as conn:
                required = ['height', 'accounts', 'nonces', 'target', 'total_tx', 'accepted', 'rejected']
                for key in required:
    ...
    def _create_genesis(self):
        g = {
            ...
            "scratchpad_mods": 0, "scratchpad_size": 8388608,
            "version": PROTOCOL
        }
    ...
    def _save_block(self, block):
        try:
            with sqlite3.connect(DB_PATH, timeout=30) as conn:
                ...
                for key, val in [('height', str(self.height)), ('accounts', json.dumps(self.accounts)), ('nonces', json.dumps(self.nonces)), ('target', str(self.target)), ('total_tx', str(self.total_tx)), ('accepted', str(self.accepted)), ('rejected', str(self.rejected))]:
    ...
    def fmt_diff(self) -> str:
        ...
        if sd >= 1e12:
            return f"{sd / 1e12:.2f} TRam/s"
        if sd >= 1e9:
            return f"{sd / 1e9:.2f} GRam/s"
        if sd >= 1e6:
            return f"{sd / 1e6:.2f} MRam/s"
        if sd >= 1e3:
            return f"{sd / 1e3:.2f} KRam/s"
    ...
    async def start_server(self, host: str, port: int):
        try:
            ...
            if self.is_white and self.bc:
                await self.seed_registry.add_seed(self.my_ip, P2P_PORT, self.bc.height, VERSION)
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    if msg.get("type") == "new_block" and self.bc:
                        block = msg.get("block")
                        if block:
                            await self.bc.add_block(block, "broadcast")
                except:
                    pass
        except:
            pass
...
async def handle_pool_share(request):
    ...
    if not a.startswith("RAM_"):
        return web.json_response({"status": "rejected"}, status=400)
    ok = await bc.submit_share(a, int(d.get("nonce", 0)), int(d.get("nonce_seed", 0)), d.get("mix", "0"), int(d.get("mods", 0)), int(d.get("extra_nonce", 0)), int(d.get("scratchpad_size", BASE_SCRATCHPAD)))
...
async def handle_seeds(request):
    ...
    if bc.p2p:
        ...
        return web.json_response({"seeds": [{"ip": s.ip, "port": s.port, "height": s.height} for s in seeds[:20]], "total": len(seeds)})
...
async def handle_network(request):
    ...
    return web.json_response({
        ...
        "is_white": p2p.is_white, "height": bc.height,
...
class Node:
    ...
    async def start(self):
        ...
        try:
            ...
            log.warning("⚠️ Метрики не запущены (порт занят?)")
