...
class Blockchain:
    ...
    def _load(self):
        try:
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                # Пересчитываем хеши для консистентности
                ...
                integrity_ok, msg = self.verify_chain_integrity()
                if not integrity_ok:
                    log.warning(f"⚠️ Цепь имеет расхождения: {msg}")
    ...
    def calc_hash(self, block):
        ...
        # Всегда считаем без miner_signature для новых блоков
    ...
    async def periodic_cleanup(self):
        ...
        while True:
            ...
            now = time.time()
            old_orphans = [
                h for h, b in self.orphans.items()
                if now - b.get('timestamp', 0) > 3600
            ]
            for h in old_orphans:
                del self.orphans[h]
            dead_clients = {ws for ws in self.ws_clients if ws.closed}
            self.ws_clients -= dead_clients
            gc.collect()
            if old_orphans:
                log.debug(f"🧹 Очищено {len(old_orphans)} орфанных блоков")

    def verify_chain_integrity(self):
        """Полная проверка целостности цепочки блоков"""
        if not self.chain:
            return True, "Цепь пуста"
        for i in range(1, len(self.chain)):
            current = self.chain[i]
            previous = self.chain[i - 1]
            if current.get('previous_hash') != previous.get('hash'):
                log.error(f"❌ Разрыв цепи на блоке #{i}")
                return False, f"Разрыв цепи на блоке #{i}"
            calculated_hash = self.calc_hash(current)
            if calculated_hash != current.get('hash'):
                log.error(f"❌ Неверный хеш блока #{i}")
                return False, f"Неверный хеш блока #{i}"
        log.info(f"✅ Целостность цепи подтверждена ({len(self.chain)} блоков)")
        return True, "OK"
