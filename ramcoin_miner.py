#!/usr/bin/env python3
"""
RAMCOIN MINER v1.0.0 — GENESIS EDITION
Ramhash v7 • Memory-hard CPU mining • Solo + Pool
"""
import hashlib
import json
import os
...
class RamcoinMiner:
                    ...
                    if self.pool_mode and proof_int <= pool_target:
                        ...
                        if resp:
                            ...
                            if status == "ok":
                                ...
                                log.info(f"\n{BO}{GR}╔══════════════════════════════════════════════╗{NC}")
                                log.info(f"{BO}{GR}║   🎉 BLOCK #{current_height} ACCEPTED! +{current_reward:.2f} RAM{NC}")
                                log.info(f"{BO}{GR}╚══════════════════════════════════════════════╝{NC}")
                            else:
                                with self.lock:
                                    self.blocks_rejected += 1
    ...
    def run(self) -> None:
        log.info(f"{BO}╔══════════════════════════════════════════════╗{NC}")
        log.info(f"{BO}║   RAMCOIN MINER v{VERSION} — GENESIS EDITION{NC}")
        log.info(f"{BO}╚══════════════════════════════════════════════╝{NC}")

        log.info("Syncing with node...")
        if not self.update_block_info():
            log.error(f"{RE}Failed to sync with node!{NC}")
            sys.exit(1)

        diff_str = fmt_diff(self.current_target)
        ...
        log.info(f"Mode: {'POOL' if self.pool_mode else 'SOLO'}")
