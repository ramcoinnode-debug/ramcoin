...
class CircuitBreaker:
    ...
    def record_failure(self, url):
        with self.lock:
            ...
            if self.failure_counts[url] >= 5:
                self.blocked_until[url] = time.time() + 30
                print(f"⚠️ Circuit breaker: {url} заблокирован на 30с")

    def record_success(self, url):
        with self.lock:
            self.failure_counts[url] = max(0, self.failure_counts.get(url, 0) - 1)
...
def safe_request(url, data=None, timeout=NODE_TIMEOUT, max_retries=MAX_RETRIES):
    if not cb.can_request(url):
        print(f"⏸️ Запрос к {url} заблокирован circuit breaker")
        return None
        ...
        while state.active:
            ...
            try:
                progress = {
                    ...
                    "height": state.height,
                    "mined": state.mined,
                    "total_ram": state.total_ram,
                    "rejected": state.rejected,
                    "pool_shares": state.pool_shares
                }
                with open("mining_progress.json", "w") as f:
