#!/usr/bin/env python3
"""
RAMCOIN NODE v7.0.3 — STABLE + POOL + BURN + ADDRESS API
Основа: v7.0.2. Исправлено: соло/пул награды разделены.
Соло: 100% майнеру.
...
class Blockchain:
    ...
    def init_db(self):
        with sqlite3.connect(DB) as conn:
            ...
            conn.execute(
                '''CREATE TABLE IF NOT EXISTS transactions (tx_hash TEXT PRIMARY KEY, block_idx INTEGER, sender TEXT, recipient TEXT, amount INTEGER, fee INTEGER, timestamp INTEGER)''')
            conn.execute(
                '''CREATE TABLE IF NOT EXISTS pool_shares (id INTEGER PRIMARY KEY AUTOINCREMENT, miner_address TEXT, shares INTEGER, timestamp INTEGER)''')
        ...
        try:
            with sqlite3.connect(DB) as conn:
                for key in ['height', 'accounts', 'nonces', 'target', 'total_tx', 'accepted', 'rejected']:
                    ...
                    if key == 'height':
                        self.height = int(val)
                    elif key in ('accounts', 'nonces'):
                        setattr(self, key, json.loads(val))
                    elif key == 'target':
                        self.target = int(val)
                    elif key == 'total_tx':
                        self.total_tx = int(val)
                    elif key == 'accepted':
                        self.accepted = int(val)
                    elif key == 'rejected':
                        self.rejected = int(val)
    ...
    def verify_pow_sync(self, block, target_override=None):
        try:
            ...
            sp, seed = self.create_scratchpad(str(block["previous_hash"]), int(block.get("extra_nonce", 0)),
            ...
            proof = hashlib.sha256(
                f"{mix}{block['previous_hash']}{block.get('nonce_seed', 0)}{mods}".encode()).hexdigest()
    ...
    def verify_miner_sig(self, block):
        try:
            addr = block.get("miner_payout_address", "");
            sig = block.get("miner_signature", "")
    ...
    def verify_tx(self, tx):
        try:
            ...
            pub.verify(bytes.fromhex(tx.get("signature", "")), json.dumps(tx_data, sort_keys=True).encode(),
    ...
    def distribute_pool_reward(self):
        """Распределяет награду пула: 98% майнерам, 1% dev, 1% burn"""
    ...
    async def add_block(self, block, ip="unknown"):
        async with self.lock:
            ...
            if not prev or prev["hash"] != block[
                "previous_hash"]: self.rejected += 1; return False, "invalid_previous_hash"
            miner = block.get("miner_payout_address", DEV_ADDR)
            if not miner.startswith("RAM_") or len(
                miner) != 134: self.rejected += 1; return False, "invalid_miner_address"
            ...
            # ===== ИСПРАВЛЕНО: разделение соло и пул =====
            is_pool_block = block.get("pool_block", False)

            if is_pool_block:
                # ПУЛ-БЛОК: награда распределяется между участниками пула
                self.distribute_pool_reward()
                reward = 0  # майнер блока не получает отдельную награду, он уже в пуле
                log.info(f"Block #{idx} | POOL | {'SERVER' if is_server else 'HOME'} | Diff: {self.fmt_diff()}")
            else:
                # СОЛО-БЛОК: награда целиком майнеру
                reward = INITIAL_REWARD // (2 ** (idx // HALVING))
                if self.is_server_address(miner):
                    reward = int(reward * SERVER_PENALTY)
                self.accounts[miner] = self.accounts.get(miner, 0) + reward
                log.info(
                    f"Block #{idx} | +{reward / COIN:.2f} RAM | {miner[:16]}... | {'SERVER' if is_server else 'HOME'} | SOLO | Diff: {self.fmt_diff()}")

            for tx in block.get("transactions", []):
                s, r, a = tx["sender"], tx["recipient"], int(tx["amount"])
                df = (FIXED_FEE * DEV_SHARE) // 100;
                mf = FIXED_FEE - df
            ...
            self.mempool = [tx for tx in self.mempool if
                            tx["signature"] not in {t["signature"] for t in block.get("transactions", [])}]
            self.adjust_target();
    ...
    def get_stats(self):
        ...
        self._cache["stats"] = data;
        self._cache_ts["stats"] = now
        return data
    ...
    def get_address(self, addr):
        ...
        try:
            with sqlite3.connect(DB) as conn:
                rows = conn.execute(
                    "SELECT * FROM transactions WHERE sender=? OR recipient=? ORDER BY block_idx DESC LIMIT 20",
                    (addr, addr)).fetchall()
                for r in rows: txs.append(
                    {"tx_hash": r[0], "block_index": r[1], "sender": r[2], "recipient": r[3], "amount": r[4],
                     "fee": r[5], "timestamp": r[6]})
        except:
            pass
...
async def handle_mine(request):
    if not bc.check_rate(request.remote): return web.json_response({"status": "rejected", "reason": "rate_limit"},
                                                                   status=429)
    try:
        block = await request.json()
    except:
        return web.json_response({"status": "rejected", "reason": "invalid_json"}, status=400)
    success, reason = await bc.add_block(block, request.remote)
    return web.json_response({"status": "ok" if success else "rejected", "reason": reason},
...
async def handle_pool_share(request):
    try:
        ...
        if not data.get("miner_address", "").startswith("RAM_"): return web.json_response({"status": "rejected"},
                                                                                          status=400)
            ...
            if msg.type == web.WSMsgType.TEXT:
                try:
                    ...
                    if data.get("type") == "miner":
                        ...
                        bc.miner_stats[hw.get("address", "unknown")] = {"cpu": hw.get("cpu", ""),
                                                                        "cores": hw.get("cores", 0),
                                                                        "l3_mb": hw.get("l3_mb", 0),
...
async def heartbeat():
    while True:
        ...
        log.info(
            f"Height: {bc.height} | Diff: {bc.fmt_diff()} | Miners: {len(bc.ws_clients)} | Pool: {len(bc.pool_shares)}")
