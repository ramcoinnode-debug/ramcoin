#!/usr/bin/env python3
"""
RAMCOIN WALLET v1.0.0 - GENESIS EDITION
AES-256-GCM + PBKDF2-SHA512 + BIP39 Mnemonic
Offline transactions + Address book + Auto-nonce
"""
import hashlib
import json
import os
...
class KeyGenerator:
    WORDLIST = [
        ...
        "awkward", "axis", "baby", "bachelor", "badge", "bag", "balance",
...
def validate_address(address: str) -> Tuple[bool, str]:
    if not address.startswith("RAM_"):
        return False, "Address must start with 'RAM_'"
    if len(address) != 134:
        return False, f"Invalid length ({len(address)} != 134)"
    try:
...
class AddressBook:
    ...
    def add(name: str, address: str):
        ...
        print(f"{GREEN}Added {name} to address book{RESET}")
...
class NetworkAPI:
    ...
    def get(path: str, node: str = None) -> Optional[dict]:
        for url in ([node] if node else NODES):
            try:
...
class WalletManager:
    ...
    def create_wallet(self) -> Optional[dict]:
        print(f"\n{CYAN}{BOLD}CREATE NEW WALLET{RESET}")
        mnemonic = KeyGenerator.generate_mnemonic()
        print(f"\n{YELLOW}SAVE THESE 12 WORDS (they are not stored!):{RESET}")
        print(f"{BOLD}{mnemonic}{RESET}\n")
        input("Press ENTER when you have saved them...")
        ...
        while True:
            ...
            pwd = input("> ").strip()
            if len(pwd) < 8:
                print(f"{RED}Too short!{RESET}")
                continue
            if pwd != input("Repeat password: ").strip():
        ...
        print(f"\n{GREEN}{BOLD}Wallet created!{RESET}")
        print(f"Address: {CYAN}{address}{RESET}")
        return wallet_data

    def load_wallet(self) -> Optional[dict]:
        if not os.path.exists(WALLET_FILE):
            print(f"{RED}Wallet file not found!{RESET}")
            return None
        ...
        try:
            ...
            print(f"\nWallet: {wf.get('address', 'Unknown')[:42]}...")
            for attempt in range(3):
                pwd = input(f"Password ({attempt + 1}/3): ").strip()
                if not pwd:
                    continue
                decrypted = WalletCrypto.decrypt(wf["crypto_data"], pwd)
                if decrypted:
                    print(f"{GREEN}Wallet loaded!{RESET}")
                    return decrypted
    ...
    def send_transaction(self, sender: str, priv_hex: str, recipient: str, amount_ram: float) -> bool:
        ...
        if not valid:
            ...
            return False
        if sender == recipient:
            print(f"{RED}Cannot send to yourself!{RESET}")
            return False
        if amount_ram <= 0:
            print(f"{RED}Amount must be > 0!{RESET}")
            return False
        ...
        if balance is None:
            print(f"{RED}Node unavailable!{RESET}")
            return False
        if balance < amount + fee:
            print(f"{RED}Insufficient funds!{RESET}")
            print(f"   Available: {balance / COIN:.8f} RAM")
            print(f"   Required: {(amount + fee) / COIN:.8f} RAM")
            return False
        ...
        try:
            ...
            signature = priv_key.sign(json.dumps(signing, sort_keys=True).encode(), ec.ECDSA(hashes.SHA256()))
            tx_data["signature"] = signature.hex()
        except Exception as e:
            print(f"{RED}Signing error: {e}{RESET}")
            return False

        print(f"\nSending {amount_ram} RAM to {recipient[:30]}...")
        resp = NetworkAPI.post("/tx", tx_data, self.node_url)

        if resp and resp.get("status") == "ok":
            print(f"{GREEN}{BOLD}Transaction sent!{RESET}")
            print(f"   Amount: {amount_ram} RAM")
            print(f"   Fee: {fee / COIN} RAM")
            print(f"   Recipient: {recipient[:42]}...")
            return True
        else:
            reason = resp.get("reason", "unknown error") if resp else "no response"
...
def main_menu(wm: WalletManager):
    ...
    while True:
        ...
        print(f"{'='*55}")
        print(f"Address: {wd['address'][:42]}...")
        print(f"Network fee: {FIXED_NETWORK_FEE / COIN} RAM")
        print(f"{'-'*55}")
        print(f"1. Check balance")
        print(f"2. Send RAM")
        print(f"3. Address book")
        ...
        if choice == "1":
            ...
            if balance is not None:
                print(f"\nBalance: {GREEN}{BOLD}{balance / COIN:,.8f} RAM{RESET}")
                print(f"Nonce: {nonce}")
            else:
                print(f"{YELLOW}Node unavailable{RESET}")
            ...
            if book:
                ...
                try:
                    ...
                    if 1 <= idx <= len(list(book.keys())):
                        ...
                        recipient = input("Address (RAM_...): ").strip()
                except:
                    recipient = input("Address (RAM_...): ").strip()
            else:
                recipient = input("Recipient address (RAM_...): ").strip()

            try:
                amount = float(input("Amount (RAM): ").strip())
            except ValueError:
            ...
            with open(WALLET_FILE, 'r') as f:
                wf = json.load(f)
            pwd = input("Password to confirm: ").strip()
            ...
            print(f"\n{CYAN}ADDRESS BOOK{RESET}")
            print("1. Show | 2. Add | 3. Remove | 4. Back")
            sub = input("> ").strip()
            if sub == "1":
                AddressBook.show()
            elif sub == "2":
                name = input("  Name: ").strip()
                addr = input("  Address (RAM_...): ").strip()
            ...
            print(f"\n{CYAN}{BOLD}WALLET INFO{RESET}")
            print(f"Version: {wd.get('version', '?')}")
            print(f"Address: {wd['address']}")
            print(f"Private key: {wd.get('private_key_hex', 'hidden')}")
            print(f"Created: {time.ctime(wd.get('created_at', 0))}")
            print(f"Encryption: AES-256-GCM + PBKDF2-SHA512 (600K iterations)")
            ...
            if decrypted is None:
                print(f"{RED}Wrong password!{RESET}")
                continue
            new_pwd = input("New password (min. 8): ").strip()
            if len(new_pwd) < 8:
                print(f"{RED}Too short!{RESET}")
                continue
...
""")
    ...
    if os.path.exists(WALLET_FILE):
        ...
        if choice == "1":
            ...
            print(f"\n{CYAN}RECOVERY FROM MNEMONIC{RESET}")
            mnemonic = input("Enter 12 words separated by spaces: ").strip()
            if len(mnemonic.split()) < 12:
                print(f"{RED}Need exactly 12 words!{RESET}")
                return
            priv_hex, address = KeyGenerator.mnemonic_to_keys(mnemonic)
            print(f"\nWallet recovered!")
        ...
        print(f"{YELLOW}Wallet not found. Creating new...{RESET}")
        wm.wallet_data = wm.create_wallet()
        if wm.wallet_data:
            main_menu(wm)
